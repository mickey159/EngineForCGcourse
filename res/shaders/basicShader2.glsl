...
vec3 colorcalc(vec3 objectColor){
	...
	vec3 lightDir = vec3(0.5, 0.0, -1.0);
	int specpower = 20;
	float kd = calcdiffuse(normal0, lightDir);
	float ks = calcspecular(normal0, lightDir, viewdir, specpower);
	vec3 ambient = vec3(0.3, 0.4, 0.5);
	vec3 retcolor = 20 * ambient * (kd + ks) * objectColor;
